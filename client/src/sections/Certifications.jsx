...
function ProviderIcon({ provider }) {
  ...
  if (provider === 'Atlassian') return (
    <svg width="22" height="22" viewBox="0 0 26 26" fill="none" aria-hidden="true">
      <path d="M5.5 20.5C8.2 15 10.5 11 13 7c2.5 4 4.8 8 7.5 13.5H5.5z" stroke="#2f6fe0" strokeWidth="2" fill="none" strokeLinejoin="round"/>
      <path d="M8.5 15.5C10 13 11.5 11 13 9c1.5 2 3 4 4.5 6.5H8.5z" fill="#2f6fe0" opacity="0.5"/>
    </svg>
  );
  ...
  if (provider === 'ISO') return (
    <svg width="22" height="22" viewBox="0 0 26 26" fill="none" aria-hidden="true">
      <rect x="4" y="9" width="18" height="8" rx="2" stroke="#7d8a96" strokeWidth="2"/>
      <line x1="9" y1="9" x2="9" y2="5" stroke="#7d8a96" strokeWidth="2" strokeLinecap="round"/>
      <line x1="17" y1="9" x2="17" y2="5" stroke="#7d8a96" strokeWidth="2" strokeLinecap="round"/>
    </svg>
  );
  return (
    <svg width="22" height="22" viewBox="0 0 26 26" fill="none" aria-hidden="true">
...
export default function Certifications() {
  ...
  useEffect(() => {
    getCertifications().then((data) => { if (data.length) setCerts(data); }).catch(() => {});
  }, []);
      ...
      <div className="container">
        <div className={styles.head}>
          <div>
            ...
            <h2 className="h2">Certified on the <span className="gradient-text">platforms</span> we ship on</h2>
          </div>
        ...
        <div className={styles.grid}>
          {certs.map((c) => (
            <div key={c.id} className={styles.card} style={{ '--c': PROVIDER_ACCENT[c.provider] || '#7d8a96' }}>
              <div className={styles.top}>
                <span className={styles.provider}>{c.provider}</span>
                <ProviderIcon provider={c.provider} />
              </div>
              <h3 className={styles.title}>{c.title}</h3>
              {c.issued_date && <span className={styles.date}>{c.issued_date}</span>}
            </div>
          ))}
        </div>
      </div>
